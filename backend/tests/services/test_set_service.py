...
class TestSetService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch(
            "src.services.set_service.SetRepository",
            return_value=self.set_repository_mock,
        ):
    ...
    def test_update_set(self):
        ...
        self.set_repository_mock.get_set.side_effect = [
            original_set_data,
            updated_set_data,
        ]

        # Update data to pass to service
        update_data = {"reps": 12, "weight": 245.0, "rpe": 9, "notes": "Updated notes"}
        ...
        self.set_repository_mock.get_set.assert_any_call(
            "set123"
        )  # Called once before update
        self.set_repository_mock.update_set.assert_called_once_with(
            "set123", update_data
        )
        self.set_repository_mock.get_set.assert_any_call(
            "set123"
        )  # Called again after update
    ...
    def test_delete_sets_for_exercise(self):
        ...
        # Assert
        self.set_repository_mock.delete_sets_by_exercise.assert_called_once_with(
            "ex123"
        )
