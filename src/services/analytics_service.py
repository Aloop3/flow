...
class AnalyticsService:
    ...
    def get_max_weight_history(self, athlete_id: str, exercise_type: str) -> List[Dict[str, Any]]:
        """
        ...
        completed_exercises = self.workout_repository.get_completed_exercises_by_type(athlete_id, exercise_type)
        ...
        max_weight_data = [{"date": date,  "max_weight": weight} for date, weight in max_weight_by_date.items()]
    ...
    def calculate_volume(self, athlete_id: str, time_period: str) -> List[Dict[str, Union[str, float]]]:
        """
        ...
        completed_workouts = self.workout_repository.get_completed_workouts_since(athlete_id, start_date)
        ...
        for exercise in workout.get("exercises", []):
            if all(k in exercise for k in ["actual_sets", "actual_reps", "actual_weight"]):
                exercise_volume = (exercise["actual_sets"] * exercise["actual_reps"] * exercise["actual_weight"])
        ...
        result = [{"date": date, "volume": volume} for date, volume in volume_data.items()]
    ...
    def calculate_block_volume(self, block_id: str) -> Dict[str, Any]:
            ...
            if day_data:
                ...
                for exercise in workout.get("exercises", []):
                    if all(k in exercise for k in ["actual_sets", "actual_reps", "actual_weight"]):
                        ...
                        exercise_volume: float = (exercise["actual_sets"] * exercise["actual_reps"] * exercise["actual_weight"])
        ...
        for week in weeks:
            ...
            if week_id in weekly_volumes:
                week_details[week_id] = {"week_number": week.get("week_number", 0), "volume": weekly_volumes[week_id]}
    ...
    def compare_blocks(self, block_id1: str, block_id2: str) -> Dict[str, Any]:
        ...
        volume_percent_change = (volume_diff / total_volume1 * 100) if total_volume1 > 0 else 0
        
        # Compare exercise volumes
        exercise_comparison = {}
        all_exercises = set(list(block1_analysis["exercise_volumes"].keys()) + list(block2_analysis["exercise_volumes"].keys()))
