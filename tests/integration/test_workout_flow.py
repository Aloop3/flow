...
class TestWorkoutFlow(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.lambda_context = type('LambdaContext', (), {
    ...
    def test_complete_workout_flow(self):
        ...
        with patch('src.api.user_api.user_service.create_user', side_effect=[athlete_user, coach_user]), \
             patch('src.api.block_api.block_service.create_block', return_value=block), \
             patch('src.api.week_api.week_service.create_week', return_value=week), \
             patch('src.api.day_api.day_service.create_day', return_value=day), \
             patch('src.api.workout_api.workout_service.log_workout', return_value=self.create_mock_workout()):
