...
class TestHandler(unittest.TestCase):
    ...
    def test_find_handler_with_complex_matching(self):
        ...
        handler.ROUTE_CONFIG["GET /users/{user_id}/workouts/{workout_id}"] = mock_handler
        ...
        try:
            ...
            result = handler.find_handler(method, different_path, different_resource, {"user_id": "123"})
            self.assertIsNone(result)
            
            # Test path parts mismatch
            handler.ROUTE_CONFIG.clear()
            handler.ROUTE_CONFIG["GET /users/{user_id}/workouts/{workout_id}"] = mock_handler
            mismatched_path = "/users/123/orders/456"  # Different path segment (orders vs workouts)
            mismatched_resource = "/users/{user_id}/orders/{order_id}"
            
            result = handler.find_handler(method, mismatched_path, mismatched_resource, 
    ...
    def test_find_handler_complex_path_match(self):
        ...
        try:
            ...
            result = handler.find_handler("GET", "/blocks/456/weeks", "/blocks/{block_id}/weeks", {"block_id": "456"})
            self.assertEqual(result, mock_handler)
            ...
            result = handler.find_handler("GET", actual_path, "/any/different/resource", {"user_id": "789"})
            self.assertEqual(result, mock_handler)
        finally:
