...
class TestWeekServiceParametrized(BaseTest):
        ...
        for block_id, week_number, notes, expected_result in test_cases:
            with self.subTest(block_id=block_id, week_number=week_number, notes=notes):
                ...
                if expected_result:
                    ...
                    self.week_repository_instance.create_week.assert_called_once_with(week.to_dict())
    ...
    def test_update_week_scenarios(self):
        ...
        test_cases = [
            ...
            ({"notes": "Updated notes"}, "Original notes", "Updated notes"),  # Update notes only
            ({"week_number": 5}, "Original notes", "Original notes"),  # Update week_number only, notes unchanged
            ({"notes": "New notes", "week_number": 10}, "Original notes", "New notes")  # Update both
        ]
        ...
        for update_data, original_notes, expected_notes in test_cases:
            with self.subTest(update_data=update_data):
                ...
                self.week_repository_instance.update_week.return_value = {"ResponseMetadata": {"HTTPStatusCode": 200}}
                ...
                if "week_number" in update_data:
                    ...
                    self.assertEqual(result.week_number, original_week_data["week_number"])
                
                self.assertEqual(result.notes, expected_notes)
                
                # Verify repository was called correctly
                self.week_repository_instance.update_week.assert_called_once_with("week123", update_data)
    ...
    def test_delete_week_scenarios(self):
        ...
        test_cases = [
            ...
            (3, {"ResponseMetadata": {"HTTPStatusCode": 200}}, True),  # Successful delete with 3 days
            (0, {"ResponseMetadata": {"HTTPStatusCode": 200}}, True),  # Successful delete with no days
            (2, None, False),  # Failed delete
        ]
        
        for days_deleted, delete_response, expected_result in test_cases:
            with self.subTest(days_deleted=days_deleted, delete_response=delete_response):
                # Reset mocks
                self.day_repository_instance.delete_days_by_week.reset_mock()
                self.week_repository_instance.delete_week.reset_mock()
                
                # Configure mock behavior
                self.day_repository_instance.delete_days_by_week.return_value = days_deleted
                ...
                self.day_repository_instance.delete_days_by_week.assert_called_once_with("week123")
                self.week_repository_instance.delete_week.assert_called_once_with("week123")